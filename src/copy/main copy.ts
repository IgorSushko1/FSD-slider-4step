import { View_horizontal } from "./view_horizontal copy";
import { View_vertical } from "./view_vertical copy";
import { Model } from "./model copy";
import { Controller } from "./controller copy";
...
for (let key_one in obj_fixed_values) {
	...
	//
	// document.getElementById(settings._element_id).appendChild(div).appendChild(select);
//
...
//
	// let parent = document.querySelector(settings._element_id);
	// let test_field = document.createElement("div");
	// test_field.className = "doc_panel";
//
