...
class ViewHorizontal {
  ...
  private writeDoubleSliderIndent = () => {
    ...
    const positions = elems.forEach((element: HTMLElement) => {
      const elemBounding = element.getBoundingClientRect();
      if (this.sliderType === 'horizontal') {
        return elemBounding.left;
      } if (this.sliderType === 'vertical') {
        return elemBounding.top;
      }
    });
    [this.lowerSliderPosition, this.upperSliderPosition] = positions;
  }
  ...
  setTargetSliderPosition = () => {
    if (this.directionType === 'horizontal') {
      this.targetSliderPosition = function targetSliderPosition(num: number) {
  ...
  moveEventWithHoldMouse = (_e: MouseEvent) => {
    ...
    this.targetSliderPosition(finalPositionInPixel);
  ...
  private setHorizontalRibbonVariables = () => {
    const sliderInDOM = this.checkElementsInDOM();
    if (sliderInDOM.length === 2) {
  ...
  private setVerticalRibbonVariables = () => {
    const sliderInDOM = this.checkElementsInDOM();
    if (sliderInDOM.length === 2) {
