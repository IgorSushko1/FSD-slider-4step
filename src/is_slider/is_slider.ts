...
function is_slider(param: Param) {
  ...
  for (let key_one in obj_fixed_values) {
    ...
    document
      .getElementById(settings.settings_id)
      .appendChild(div)
      .appendChild(select);

    let parent = document.getElementById(settings.settings_id);

    let element_i_choose = parent.querySelector(
      '#' + obj_fixed_values[key_one].key_word
    ) as HTMLElement;
  ...
  for (let key_one in obj_changeable_values) {
    ...
    let element_i_choose = parent.querySelector(
      '#iss__' + obj_changeable_values[key_one].key_word
    ) as HTMLElement;
